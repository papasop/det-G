"""
K1 Throttle Suite V4.3 — preregistered negative-flow replication
...
class CFG:
    seed:int=20260719
    seeds:tuple=(10103,10301,10501,10709,10903,11113,11311,11503,11701,11909)
    model_name:str="distilgpt2"  # change to "gpt2" after screening
    seq_len:int=96
    ...
    outdir:str="k1_throttle_v43_negative_replication_results"
...
stable_wins=all(comparisons[f"test:lorentz_neg-{c}"]["wins"]>=7
                for c in ("residual","euclid_neg","random_neg"))
audit={"protocol":"fixed-negative-sign; 10 paired seeds; no post-hoc sign selection",
       ...
       "G7_wins_at_least_7_of_10_vs_all":stable_wins,
       "PASS_NEGATIVE_LORENTZ_SPECIFIC":bool(matched and id_res and id_euc and id_rnd and
          ood_noninferior and minimum_effect and stable_wins)}
print("\n"+"="*100+"\nK1 TOKEN-COLLAPSE V4.3 PREREGISTERED NEGATIVE-FLOW REPLICATION\n"+"="*100)
print(json.dumps(audit,indent=2)); json.dump(audit,open(Path(cfg.outdir)/"audit_v43.json","w"),indent=2)
...
ax[1].axhline(0,color="black",lw=1); ax[1].set_title("Lorentz-negative paired test loss delta")
ax[1].set_ylabel("Lorentz − control (lower is better)")
plt.tight_layout(); plt.savefig(Path(cfg.outdir)/"v43_replication.png",dpi=180,bbox_inches="tight"); plt.show()
