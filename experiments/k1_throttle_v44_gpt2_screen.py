"""
K1 Throttle Suite V4.4-screen — frozen-protocol GPT-2 replication
...
results=[]; audits={}
variants=["residual","lorentz_neg","euclid_neg","random_neg"]
...
# Preregistered paired comparisons. Delta < 0 favors the first model.
...
def paired_delta(a_name,b_name,split):
    ...
    d=(a-b).dropna().values; n=len(d)
    m=float(d.mean()); sd=float(d.std(ddof=1)) if n>1 else 0.0
    tc=TCRIT95.get(n-1,1.96); half=tc*sd/math.sqrt(max(1,n))
    dz=m/(sd+1e-12)
    return {"values":d.tolist(),"n":n,"mean":m,"sd":sd,"paired_dz":dz,
            "ci95_t":[m-half,m+half],"wins":int((d<0).sum())}

comparisons={}
for split in ("test","ood"):
    for control in ("residual","euclid_neg","random_neg"):
        comparisons[f"{split}:lorentz_neg-{control}"]=paired_delta("lorentz_neg",control,split)
    comparisons[f"{split}:euclid_neg-residual"]=paired_delta("euclid_neg","residual",split)
    comparisons[f"{split}:random_neg-residual"]=paired_delta("random_neg","residual",split)
...
def upper_below(key,x=0.0): return comparisons[key]["ci95_t"][1]<x
id_res=upper_below("test:lorentz_neg-residual")
id_euc=upper_below("test:lorentz_neg-euclid_neg")
id_rnd=upper_below("test:lorentz_neg-random_neg")
ood_noninferior=all(upper_below(f"ood:lorentz_neg-{c}",cfg.ood_tolerance_nats)
                    for c in ("residual","euclid_neg","random_neg"))
minimum_effect=comparisons["test:lorentz_neg-residual"]["mean"]<=-1e-4
stable_wins=all(comparisons[f"test:lorentz_neg-{c}"]["wins"]==len(cfg.seeds)
                for c in ("residual","euclid_neg","random_neg"))
directional_go=(matched and minimum_effect and
    comparisons["test:lorentz_neg-residual"]["wins"]==len(cfg.seeds) and
    comparisons["test:lorentz_neg-random_neg"]["wins"]==len(cfg.seeds) and
    ood_noninferior)
audit={"protocol":"V4.4 GPT-2 screen; frozen negative sign; first 3 prespecified paired seeds; no post-hoc selection",
       "test_loss":{v:meanloss(v,"test") for v in variants},
       "ood_loss":{v:meanloss(v,"ood") for v in variants},
       "paired_comparisons":comparisons,
       "G1_pre_mix_branches_matched":matched,
       "G2_lorentz_beats_residual_95CI":id_res,
       "G3_lorentz_beats_euclid_95CI":id_euc,
       "G4_lorentz_beats_random_95CI":id_rnd,
       "G5_OOD_noninferior_to_all":ood_noninferior,
       "G6_minimum_effect_1e-4_nats":minimum_effect,
       "G7_wins_all_3_screen_seeds_vs_all":stable_wins,
       "GO_EXTEND_TO_10_SEEDS":bool(directional_go),
       "PASS_V44_GPT2_SCREEN":bool(matched and id_res and id_euc and id_rnd and
          ood_noninferior and minimum_effect and stable_wins)}
print("\n"+"="*100+"\nK1 TOKEN-COLLAPSE V4.4 GPT-2 FROZEN-PROTOCOL SCREEN\n"+"="*100)
print(json.dumps(audit,indent=2)); json.dump(audit,open(Path(cfg.outdir)/"audit_v44_screen.json","w"),indent=2)
...
for key,q in comparisons.items():
    if key.startswith("test:lorentz_neg-"):
        control=key.split("-")[-1]
...
ax[1].axhline(0,color="black",lw=1); ax[1].set_title("GPT-2 Lorentz-negative paired test loss delta")
ax[1].set_ylabel("Lorentz − control (lower is better)")
plt.tight_layout(); plt.savefig(Path(cfg.outdir)/"v44_gpt2_screen.png",dpi=180,bbox_inches="tight"); plt.show()
